...
namespace PruebaCoink.Application.UseCases.UseCases.User.Queries.GetByIdQuery
{
    public class GetUsersByIdHandler : IRequestHandler<GetUsersByIdQuery, BaseResponse<GetUserByIdResponseDto>>
    {
        ...
        public async Task<BaseResponse<GetUserByIdResponseDto>> Handle(GetUsersByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var patient = await _unitOfWork.User.GetByIdAsync(SP.SP_UserById, request);
                switch (patient)
                {
                    case null:
                    ...
                    default:
                        ...
                        response.Data = _mapper.Map<GetUserByIdResponseDto>((Entity.User?)await _unitOfWork.User.GetByIdAsync(SP.SP_UserById, request));
